set LSB_JOBNAME=59:lsfadmin:testflow21:testflow2:testjob2
set LSB_JOBID=123
@echo off
REM -----------------------------------------------------------------
...
IF DEFINED LSB_JOBID (
  IF DEFINED LSB_JOBNAME (
    ...
    SET DIMON_SASLOGFILE_PREPEND_JOBID_FLOWID_USER=NO
    SET DIMON_SASLOGFILE_APPEND_DATETIME=NO
    SET DIMON_CONTINUEONFAIL=YES
    SET DIMON_DEBUG=NO

    REM overwrite default options with options from dimon_usermods.sh
    ...
    REM set directory containing DIMon SAS programs
    ...
    REM -----------------------------------------------------------------
    "%SAS_COMMAND%" -sysin "!DIMON_SCRIPTDIR!/dimon_job_start.sas" -log "!DIMON_JOB_START_LOG!" -set LSB_JOBNAME "!LSB_JOBNAME!" !DIMON_SETSASLOGFILE! !DIMON_SETSASLSTFILE! -set LSB_JOBID "!DIMON_LSF_JOBID!"
    SET DIMON_RC=%ERRORLEVEL%
    IF !DIMON_RC! GTR 1 (
      ECHO ERROR: Registering DIMON Job Start event failed for job "!DIMON_LSF_JOBNAME!". RC=!DIMON_RC!
      ...
      IF "!DIMON_CONTINUEONFAIL!" NEQ "YES" (
        ...
        IF "!DIMON_DEBUG!" NEQ "YES" (
          REM remove dimon_job_start logfile if we are not in debug mode
...
ENDLOCAL & SET DIMON_CMDLINEARGS=%DIMON_CMDLINEARGS%
