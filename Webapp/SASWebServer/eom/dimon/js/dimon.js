...
$(document).click(function(event) {
    ...
    if (target) {
        if( (target.id != 'menubuttonNavigate') && ($(target).closest("#menuNavigate").attr('id') != 'menuNavigate') ) {
            $('#menuNavigate').remove();
        }
...
$(function() {
    ...
    $.ajax({       url : settings.urlSPA
           ...
           ,   success : function(data) {
                           navigateMenuItems = $.parseJSON(data);
                           console.log(navigateMenuItems);
                         }
           ,     error : function(XMLHttpRequest,textStatus,errorThrown) {
...
function createNavigateMenu() {
  ...
  var menuNavigate = $('<div id="menuNavigate" style="display:block;'
                                                ...
                                                +'" class="dropdown-menu"></div>').appendTo('body');
  $("#menuNavigate").html(s);
  $('.li-dropdown-navigate-item').click( function () { navigate2($(this).attr('id').split('-')[1]); });

}//createNavigateMenu


function navigate2(i) {
  $("#menuNavigate").remove();
  window.location.href = navigateMenuItems[i].url;
}
